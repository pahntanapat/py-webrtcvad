...
c_sources = ([os.path.join('cbits', 'pywebrtcvad.c')] + glob.glob(
    os.path.join(C_SRC_PREFIX, 'signal_processing', '*.c')) +
             glob.glob(os.path.join(C_SRC_PREFIX, 'third_party', '*.c')) +
             glob.glob(os.path.join(C_SRC_PREFIX, 'vad', '*.c')) +
             glob.glob(os.path.join(C_SRC_RTC_PREFIX, 'checks.cc')))
...
setup(
    ...
    extras_require={
        'dev': [
            'nose', 'check-manifest', 'unittest2', 'zest.releaser', 'psutil',
            'memory_profiler'
        ]
    })
